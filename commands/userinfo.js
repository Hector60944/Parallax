exports.run = async (ctx) => {
    ...
    if (member) {
        ...
        kicks = kicks ? kicks.entries.filter(e => e.targetType === 'USER' && e.target.id === user.id).size : 'UNKNOWN';
        bans  = bans  ? bans.entries.filter(e => e.targetType === 'USER' && e.target.id === user.id).size  : 'UNKNOWN';
